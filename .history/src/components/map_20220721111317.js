...
const Map = () => {
    ...
    const onClickMap = useCallback((e) => {
      ...
      if (state.currentStep === CLICK_MAP_STEP.start) {
        ...
        if(state.startCords && state.endCords) {
          updateRoute();
        }

        console.log("Map: ", map.current.getSource("end"), map.current.getSource("start"));
        
    }, [state.currentStep, state.endCords, state.startCords, updateRoute]);
